			    ...
			    enum mtd_dump_reason reason)
{
	...
	if((enum kmsg_dump_reason)reason == KMSG_DUMP_SHUTDOWN ||
		(enum kmsg_dump_reason)reason == KMSG_DUMP_EMERG)
		return;

	/* Only dump oopses if dump_oops is set */
	if ((enum kmsg_dump_reason)reason == KMSG_DUMP_OOPS && !dump_oops)
		return;
	...
	if ((enum kmsg_dump_reason)reason == KMSG_DUMP_OOPS || (enum kmsg_dump_reason)reason == KMSG_DUMP_PANIC) {
